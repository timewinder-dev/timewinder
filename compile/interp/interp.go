...
loop:
	for {
		...
		case vm.EQL, vm.NEQ, vm.GT, vm.LT, vm.LE, vm.GE:
			op := syntax.Token(op-vm.EQL) + syntax.EQL
			y := stack[sp-1]
			x := stack[sp-2]
			sp -= 2
		...
		case vm.PLUS,
			...
			vm.GTGT,
			vm.IN:
			binop := syntax.Token(op-vm.PLUS) + syntax.PLUS
			if op == vm.IN {
		...
		case vm.UPLUS, vm.UMINUS, vm.TILDE:
			var unop syntax.Token
			if op == vm.TILDE {
				unop = syntax.TILDE
			} else {
				unop = syntax.Token(op-vm.UPLUS) + syntax.PLUS
			}
			x := stack[sp-1]
			y, err2 := Unary(unop, x)
			if err2 != nil {
		...
		case vm.CALL, vm.CALL_VAR, vm.CALL_KW, vm.CALL_VAR_KW:
			var kwargs Value
			if op == vm.CALL_KW || op == vm.CALL_VAR_KW {
				kwargs = stack[sp-1]
				sp--
			}

			var args Value
			if op == vm.CALL_VAR || op == vm.CALL_VAR_KW {
				args = stack[sp-1]
				sp--
			}
		...
		case vm.SETDICT, vm.SETDICTUNIQ:
			...
			if op == vm.SETDICTUNIQ && dict.Len() == oldlen {
